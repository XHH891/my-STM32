...
uint16_t ad_getvalue(uint8_t adc_channel)//返回转换结果 
{
...
int main(void)
{
	...
	while(1)
	{
		a0 = ad_getvalue(ADC_Channel_0);
		a1 = ad_getvalue(ADC_Channel_1);
		a2 = ad_getvalue(ADC_Channel_2);
		a3 = ad_getvalue(ADC_Channel_3);
		
		OLED_ShowNum(1,5,a0,4);
		OLED_ShowNum(2,5,a1,4);
		OLED_ShowNum(3,5,a2,4);
		OLED_ShowNum(4,5,a3,4);
	}
}